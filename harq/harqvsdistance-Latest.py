...
bins = list(range(0, 250 + 15, 15))  # 50m intervals up to 250 meters
...
# Get the x-tick labels (distance bins) and show only every other bin
x_ticks = grouped_data['distance_bin'].astype(str)
skip_labels = [label if i % 2 == 0 else '' for i, label in enumerate(x_ticks)]  # Show label for every other bin

# Apply filtered x-tick labels
plt.xticks(ticks=np.arange(len(x_ticks)), labels=skip_labels, rotation=45, fontsize=18)
